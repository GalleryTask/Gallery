...
//最大放大倍数
static const CGFloat ItemMaxScale = 1.0;
...
- (void)buildUI {
    ...
    _bottomLine.backgroundColor = BASECOLOR_BACKGROUND_GRAY;
...
- (void)layoutSubviews {
    ...
    _bottomLine.frame = CGRectMake(0, self.bounds.size.height - 10, self.bounds.size.width, 10);
...
- (CGRect)shadowRectOfIndex:(NSInteger)index {
    return CGRectMake([_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]].frame.origin.x, self.bounds.size.height - 13, [self itemWidthOfIndexPath:[NSIndexPath indexPathForRow:index inSection:0]], 3);
}
